...
impl MarkerDemo {
	...
	fn ui(&mut self, ui:&mut Ui) -> Response {
		...
		let markers_plot = Plot::new("markers_demo").data_aspect(1.0).legend(Legend::default());
...
impl InteractionDemo {
	...
	fn ui(&mut self, ui:&mut Ui) -> Response {
		...
		ui.label(format!("plot bounds: min: {:.02?}, max: {:.02?}", bounds.min(), bounds.max()));
