...
impl Default for MultiTouch {
	fn default() -> Self {
		Self { rotation:0., translation:Vec2::ZERO, zoom:1., last_touch_time:0.0 }
	}
}

impl super::Demo for MultiTouch {
	fn name(&self) -> &'static str { "👌 Multi Touch" }
...
impl super::View for MultiTouch {
	fn ui(&mut self, ui:&mut egui::Ui) {
		...
		let color = if ui.visuals().dark_mode { Color32::WHITE } else { Color32::BLACK };

		Frame::canvas(ui.style()).show(ui, |ui| {
			// Note that we use `Sense::drag()` although we do not use any pointer events.
			// With the current implementation, the fact that a touch event of two or
			// more fingers is recognized, does not mean that the pointer events are
			// suppressed, which are always generated for the first finger. Therefore, if
			// we do not explicitly consume pointer events, the window will move around,
			// not only when dragged with a single finger, but also when a two-finger
			// touch is active. I guess this problem can only be cleanly solved when the
			// synthetic pointer events are created by egui, and not by the backend.

			// set up the drawing canvas with normalized coordinates:
			let (response, painter) =
				ui.allocate_painter(ui.available_size_before_wrap(), Sense::drag());

			// normalize painter coordinates to ±1 units in each direction with [0,0] in the
			// center:
			...
			// check for touch input (or the lack thereof) and update zoom and scale
			// factors, plus color and width:
			let mut stroke_width = 1.;
			if let Some(multi_touch) = ui.ctx().multi_touch() {
				// This adjusts the current zoom factor and rotation angle according to the
				// dynamic change (for the current frame) of the touch gesture:
				...
				// touch pressure will make the arrow thicker (not all touch devices support
				// this):
			...
			// Paints an arrow pointing from bottom-left (-0.5, 0.5) to top-right (0.5,
			// -0.5), but scaled, rotated, and translated according to the current touch
			// gesture:
