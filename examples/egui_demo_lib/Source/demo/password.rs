//! Source code example about creating a widget which uses `egui::Memory` to
//! store UI state.
//!
...
pub fn password_ui(ui:&mut egui::Ui, password:&mut String) -> egui::Response {
	...
	// (`show_plaintext`). In this case we use a simple `bool`, but you can also
	// declare your own type. It must implement at least `Clone` and be `'static`.
	// If you use the `persistence` feature, it also must implement
	...
	// You should get state by value, not by reference to avoid borrowing of
	// [`Memory`].
	let mut show_plaintext = ui.data().get_temp::<bool>(state_id).unwrap_or(false);

	// Process ui, change a local copy of the state
	// We want TextEdit to fill entire space, and have button after that, so in that
	// case we can change direction to right_to_left.
