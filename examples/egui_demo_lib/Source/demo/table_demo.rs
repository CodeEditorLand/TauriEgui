...
impl Default for TableDemo {
	fn default() -> Self { Self { demo:DemoType::Manual, resizable:true, num_rows:10_000 } }
}

impl super::Demo for TableDemo {
	fn name(&self) -> &'static str { "☰ Table Demo" }
				...
				match self.demo {
					...
					DemoType::ManyHeterogenous => {
						fn row_thickness(row_index:usize) -> f32 {
							if thick_row(row_index) { 30.0 } else { 18.0 }
						}
...
fn clock_emoji(row_index:usize) -> String {
	char::from_u32(0x1F550 + row_index as u32 % 24).unwrap().to_string()
}
