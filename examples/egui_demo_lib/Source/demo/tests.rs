...
impl super::Demo for CursorTest {
	fn name(&self) -> &'static str { "Cursor Test" }
	...
	fn ui(&mut self, ui:&mut egui::Ui) {
		ui.vertical_centered_justified(|ui| {
			...
			for &cursor_icon in &egui::CursorIcon::ALL {
				let _ = ui.button(format!("{:?}", cursor_icon)).on_hover_cursor(cursor_icon);
			}
...
impl super::Demo for IdTest {
	fn name(&self) -> &'static str { "ID Test" }
...
impl super::View for IdTest {
	fn ui(&mut self, ui:&mut egui::Ui) {
		...
		ui.label(
			"\
            Widgets that store state require unique and persisting identifiers so we can track \
			 their state between frames.\nFor instance, collapsable headers needs to store \
			 whether or not they are open. Their Id:s are derived from their names. If you fail \
			 to give them unique names then clicking one will open both. To help you debug this, \
		...
		ui.label(
			"\
            Any widget that can be interacted with also need a unique Id. For most widgets the Id \
			 is generated by a running counter. As long as elements are not added or removed, the \
			 Id stays the same. This is fine, because during interaction (i.e. while dragging a \
			 slider), the number of widgets previously in the same window is most likely not \
			 changing (and if it is, the window will have a new layout, and the slider will endup \
			 somewhere else, and so aborthing the interaction probably makes sense).",
		);
...
impl super::Demo for ManualLayoutTest {
	fn name(&self) -> &'static str { "Manual Layout Test" }
...
impl super::View for ManualLayoutTest {
	fn ui(&mut self, ui:&mut egui::Ui) {
		...
		let Self { widget_offset, widget_size, widget_type, text_edit_contents } = self;
...
impl Default for TableTest {
	fn default() -> Self {
		Self { num_cols:4, num_rows:4, min_col_width:10.0, max_col_width:200.0, text_length:10 }
	}
}

impl super::Demo for TableTest {
	fn name(&self) -> &'static str { "Table Test" }
...
impl Default for WindowResizeTest {
	fn default() -> Self { Self { text:crate::LOREM_IPSUM_LONG.to_owned() } }
}

impl super::Demo for WindowResizeTest {
	fn name(&self) -> &'static str { "↔ Window Resize" }

	fn show(&mut self, ctx:&egui::Context, open:&mut bool) {
		use egui::*;

		Window::new("↔ auto-sized").open(open).auto_sized().show(ctx, |ui| {
		...
		Window::new("↔ resizable + scroll")
			...
			.show(ctx, |ui| {
				ui.label(
					"This window is resizable and has a scroll area. You can shrink it to any \
					 size.",
				);
		...
		Window::new("↔ resizable + embedded scroll")
			...
			.show(ctx, |ui| {
				...
				// ui.heading("Some additional text here, that should also be
				// visible"); // this works, but messes with the resizing a bit
			});
		...
		Window::new("↔ resizable without scroll")
			...
			.show(ctx, |ui| {
				ui.label(
					"This window is resizable but has no scroll area. This means it can only be \
					 resized to a size where all the contents is visible.",
				);
		...
		Window::new("↔ freely resized")
			...
			.show(ctx, |ui| {
				ui.label(
					"This window has empty space that fills up the available space, preventing \
					 auto-shrink.",
				);
