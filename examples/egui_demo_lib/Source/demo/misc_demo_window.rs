...
impl Demo for MiscDemoWindow {
	fn name(&self) -> &'static str { "✨ Misc Demos" }
...
impl View for MiscDemoWindow {
	fn ui(&mut self, ui:&mut Ui) {
		...
		CollapsingHeader::new("Widgets").default_open(true).show(ui, |ui| {
			self.widgets.ui(ui);
		});

		CollapsingHeader::new("Text layout").default_open(false).show(ui, |ui| {
			text_layout_ui(
				ui,
				&mut self.max_rows,
		...
		CollapsingHeader::new("Checkboxes").default_open(false).show(ui, |ui| {
		...
		CollapsingHeader::new("Resize").default_open(false).show(ui, |ui| {
		...
		CollapsingHeader::new("Misc").default_open(false).show(ui, |ui| {
			ui.horizontal(|ui| {
			...
			.show(ui, |ui| {
				ui.horizontal_wrapped(|ui| {
					for i in 0..100 {
						...
						ui.painter().circle_filled(rect.center(), r, ui.visuals().text_color());
					}
				});
			});
	}
...
impl Default for Widgets {
	fn default() -> Self {
		Self { angle:std::f32::consts::TAU / 3.0, password:"hunter2".to_owned() }
	}
}
	...
	pub fn ui(&mut self, ui:&mut Ui) {
		...
		ui.horizontal_wrapped(|ui| {
			...
			ui.label("and tooltips.").on_hover_text(
				"This is a multiline tooltip that demonstrates that you can easily add tooltips \
			...
			ui.label(
				"The default font supports all latin and cyrillic characters (ИÅđ…), common math \
				 symbols (∫√∞²⅓…), and many emojis (💓🌟🖩…).",
			)
...
impl ColorWidgets {
	fn ui(&mut self, ui:&mut Ui) {
		...
		ui.label(
			"egui lets you edit colors stored as either sRGBA or linear RGBA and with or without \
			 premultiplied alpha",
		);

		let Self { srgba_unmul, srgba_premul, rgba_unmul, rgba_premul } = self;
...
impl Default for BoxPainting {
	fn default() -> Self {
		Self { size:vec2(64.0, 32.0), rounding:5.0, stroke_width:2.0, num_boxes:1 }
	}
}
...
impl Default for CustomCollapsingHeader {
	fn default() -> Self { Self { selected:true, radio_value:false } }
}
...
impl Tree {
	...
	pub fn ui(&mut self, ui:&mut Ui) -> Action { self.ui_impl(ui, 0, "root") }
}
...
fn text_layout_ui(
	...
	job.append(
		...
		TextFormat { color:default_color, ..Default::default() },
	);
	job.append(
		"the egui text layout engine. ",
		0.0,
		TextFormat { color:strong_color, ..Default::default() },
	);
	job.append("It supports ", 0.0, TextFormat { color:default_color, ..Default::default() });
	job.append(
		"different ",
		0.0,
		TextFormat { color:Color32::from_rgb(110, 255, 110), ..Default::default() },
	);
	job.append(
		"colors, ",
		0.0,
		TextFormat { color:Color32::from_rgb(128, 140, 255), ..Default::default() },
	);
	job.append(
		"backgrounds, ",
		0.0,
	...
	job.append(
		"fonts, ",
		0.0,
		TextFormat { font_id:FontId::monospace(14.0), color:default_color, ..Default::default() },
	);
	job.append(
		"raised text, ",
		0.0,
	...
	job.append("with ", 0.0, TextFormat { color:default_color, ..Default::default() });
	job.append(
		"underlining",
		0.0,
		TextFormat {
	...
	job.append(" and ", 0.0, TextFormat { color:default_color, ..Default::default() });
	job.append(
		"strikethrough",
		0.0,
		TextFormat {
	...
	job.append(". Of course, ", 0.0, TextFormat { color:default_color, ..Default::default() });
	job.append(
		"you can",
		0.0,
		TextFormat {
