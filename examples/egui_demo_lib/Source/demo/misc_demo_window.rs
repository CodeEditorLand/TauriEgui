...
impl Demo for MiscDemoWindow {
	...
	fn show(&mut self, ctx: &Context, open: &mut bool) {
		Window::new(self.name()).open(open).vscroll(true).hscroll(true).show(ctx, |ui| self.ui(ui));
	}
}
...
impl View for MiscDemoWindow {
	fn ui(&mut self, ui: &mut Ui) {
		...
		CollapsingHeader::new("Tree").default_open(false).show(ui, |ui| self.tree.ui(ui));
		...
		CollapsingHeader::new("Many circles of different sizes").default_open(false).show(
			ui,
			|ui| {
				ui.horizontal_wrapped(|ui| {
					for i in 0..100 {
...
impl Default for CustomCollapsingHeader {
	fn default() -> Self {
		Self { selected: true, radio_value: false }
	}
}
...
impl Tree {
	pub fn demo() -> Self {
		Self(vec![Tree(vec![Tree::default(); 4]), Tree(vec![Tree(vec![Tree::default(); 2]); 3])])
	}
