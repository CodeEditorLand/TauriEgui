...
impl super::Demo for About {
	fn name(&self) -> &'static str { "About egui" }
...
impl super::View for About {
	fn ui(&mut self, ui:&mut egui::Ui) {
		...
		ui.label(format!(
			"egui is an immediate mode GUI library written in Rust. egui runs both on the web and \
			 natively on {}{}{}. On the web it is compiled to WebAssembly and rendered with \
			 WebGL.{}",
			OS_APPLE,
			OS_LINUX,
			OS_WINDOWS,
			if cfg!(target_arch = "wasm32") {
				" Everything you see is rendered as textured triangles. There is no DOM, HTML, JS \
				 or CSS. Just Rust."
			} else {
				""
			}
...
fn about_immediate_mode(ui:&mut egui::Ui) {
	...
	ui.horizontal_wrapped(|ui| {
		...
		ui.label(
			"Immediate mode is a GUI paradigm that lets you create a GUI with less code and \
			 simpler control flow. For example, this is how you create a ",
		);
  ...
  if ui.button("Save").clicked() {
	...
	ui.label(
		"Immediate mode has its roots in gaming, where everything on the screen is painted at the \
		 display refresh rate, i.e. at 60+ frames per second. In immediate mode GUIs, the entire \
		 interface is layed out and painted at the same high rate. This makes immediate mode GUIs \
		 especially well suited for highly interactive applications.",
	);
