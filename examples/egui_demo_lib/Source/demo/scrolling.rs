...
impl super::View for Scrolling {
	fn ui(&mut self, ui:&mut Ui) {
		ui.horizontal(|ui| {
			...
			ui.selectable_value(&mut self.demo, ScrollDemo::LargeCanvas, "Scroll a large canvas");
...
impl super::View for ScrollTo {
	fn ui(&mut self, ui:&mut Ui) {
		...
		ui.horizontal(|ui| {
			...
			track_item |=
				ui.add(Slider::new(&mut self.track_item, 1..=50).text("Track Item")).dragged();
		});
		...
		ui.horizontal(|ui| {
			...
			go_to_scroll_offset |=
				ui.add(DragValue::new(&mut self.offset).speed(1.0).suffix("px")).dragged();
		});
		...
		let mut scroll_area = ScrollArea::vertical().max_height(200.0).auto_shrink([false; 2]);
		...
		ui.label(format!("Scroll offset: {:.0}/{:.0} px", current_scroll, max_scroll));
