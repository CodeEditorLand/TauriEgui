...
impl super::View for TextEdit {
	fn ui(&mut self, ui: &mut egui::Ui) {
		...
		let output = egui::TextEdit::multiline(text).hint_text("Type something!").show(ui);
		...
		let anything_selected = output.cursor_range.map_or(false, |cursor| !cursor.is_empty());
		...
		if ui.input_mut().consume_key(egui::Modifiers::COMMAND, egui::Key::Y) {
			if let Some(text_cursor_range) = output.cursor_range {
				use egui::TextBuffer as _;
				let selected_chars = text_cursor_range.as_sorted_char_range();
