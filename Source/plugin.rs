...
pub struct EguiPlugin<T:UserEvent> {
	...
	pub(crate) create_window_channel:
		(SyncSender<CreateWindowPayload>, Receiver<CreateWindowPayload>),
...
impl<T> MaybeRc<T> {
	#[allow(dead_code)]
	pub fn new(t:T) -> Self { Self::Actual(t) }
}
...
impl<T> MaybeRcCell<T> {
	#[allow(dead_code)]
	pub fn new(t:T) -> Self { Self::Actual(RefCell::new(t)) }
}
	...
	let mut paint = || {
		...
		*control_flow = if integration.should_close() {
			...
			std::time::Instant::now().checked_add(repaint_after)
		{
			...
			// technically, this might lead to some weird corner cases where the
			// user *WANTS* winit to use `WaitUntil(MAX_INSTANT)` explicitly.
			// they can roll their own egui backend impl i guess.
		...
		if !is_focused {
			...
			// But we know if we are focused (in foreground). When minimized, we
			// are not focused. However, a user may want an egui with an
			// animation in the background, so we still need to repaint quite
			// fast.
					...
					if let Some(window) = windows_lock.get_mut(&window_id) {
						...
						let handled = match event {
							...
							TaoWindowEvent::Resized(physical_size) => {
								// Resize with 0 width and height is used by
...
pub(crate) fn handle_user_message<T:UserEvent>(
	...
	if let Message::Window(window_id, message) = message {
		if let Some(glutin_window_context_opt) =
			windows.lock().unwrap().get_mut(window_id).map(|win| &mut win.inner)
		{
